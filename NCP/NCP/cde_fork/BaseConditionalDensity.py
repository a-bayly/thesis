...
class ConditionalDensity(BaseEstimator):
    ...
    def _mean_pdf(self, x_cond, n_samples=10**6):
        ...
        for i in range(x_cond.shape[0]):
            ...
            if self.ndim_y == 1:
                ...
                integral = numeric_integation(
                    func_to_integrate, n_samples_int, lower, upper
                )
            else:
    ...
    def _std_pdf(self, x_cond, n_samples=10**6, mean=None):
        ...
        if self.ndim_y == 1:  # compute with numerical integration
            ...
            for i in range(x_cond.shape[0]):
                ...
                n_samples_int, lower, upper = self._determine_integration_bounds()
                func_to_integrate = lambda y: (y - mu) ** 2 * np.squeeze(
                    self._tiled_pdf(y, x_cond[i], n_samples_int)
                )
                stds[i] = np.sqrt(
                    numeric_integation(func_to_integrate, n_samples_int, lower, upper)
                )
    ...
    def _skewness_pdf(self, x_cond, n_samples=10**6, mean=None, std=None):
        ...
        if std is None:
            std = np.reshape(
                np.sqrt(self.covariance(x_cond, n_samples=n_samples)),
                (x_cond.shape[0],),
            )
        ...
        for i in range(x_cond.shape[0]):
            ...
            func_skew = lambda y: ((y - mu) / sigm) ** 3 * np.squeeze(
                self._tiled_pdf(y, x_cond[i], n_samples_int)
            )
    ...
    def _kurtosis_pdf(self, x_cond, n_samples=10**6, mean=None, std=None):
        ...
        for i in range(x_cond.shape[0]):
            mu = np.squeeze(mean[i])
            sigm = np.squeeze(std[i])
            func_skew = lambda y: ((y - mu) ** 4 / sigm**4) * np.squeeze(
                self._tiled_pdf(y, x_cond[i], n_samples_int)
            )
    ...
    def _quantile_mc(self, x_cond, alpha=0.01, n_samples=10**6):
        ...
        x_cond = np.tile(
            x_cond.reshape((1, x_cond.shape[0], x_cond.shape[1])), (n_samples, 1, 1)
        )
    ...
    def _quantile_cdf(self, x_cond, alpha=0.01, eps=1e-8, init_bound=1e3):
        ...
        return find_root_by_bounding(
            cdf_fun, left=-init_bound, right=init_bound, eps=eps
        )

    """ CONDITONAL VALUE-AT-RISK """

    def _conditional_value_at_risk_mc_pdf(
        ...
        for i in range(x_cond.shape[0]):
            upper = float(VaRs[i])
            func_to_integrate = lambda y: y * np.squeeze(
                self._tiled_pdf(y, x_cond[i], n_samples_int)
            )
            integral = numeric_integation(
                func_to_integrate, n_samples_int, lower, upper
    ...
    def _handle_input_dimensionality(self, X, Y=None, fitting=False):
        ...
        if fitting:  # store n_dim of training data
            ...
            assert (
                X.shape[1] == self.ndim_x
            ), "X must have shape (?, %i) but provided X has shape %s" % (
                self.ndim_x,
                X.shape,
            )
            if Y is not None:
                assert (
                    Y.shape[1] == self.ndim_y
                ), "Y must have shape (?, %i) but provided Y has shape %s" % (
                    self.ndim_y,
                    Y.shape,
                )
    ...
    def plot3d(
        self, xlim=(-5, 5), ylim=(-8, 8), resolution=100, show=False, numpyfig=False
    ):
        """Generates a 3d surface plot of the fitted conditional distribution if x and y are 1-dimensional each
        ...
        """
        assert (
            self.ndim_x + self.ndim_y == 2
        ), "Can only plot two dimensional distributions"
    ...
    def _determine_mc_proposal_dist(self):
        if hasattr(self, "y_std") and hasattr(self, "y_mean"):
            ...
            return (
                np.ones(self.ndim_y) * LOC_PROPOSAL,
                np.ones(self.ndim_y) * SCALE_PROPOSAL,
            )
