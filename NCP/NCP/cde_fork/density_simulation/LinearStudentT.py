...
class LinearStudentT(BaseConditionalDensitySimulation):
    ...
    def _scale(self, X):
        return np.expand_dims(
            self.std + np.mean(self.std_slope * np.abs(X), axis=-1), axis=-1
        )

    def _dof(self, X):
        return self.dof_low + (self.dof_high - self.dof_low) * _sigmoid(
            -2 * np.mean(X, axis=-1)
        )

    def __str__(self):
        return "\nProbabilistic model type: {}\n std: {}\n n_dim_x: {}\n n_dim_y: {}\n".format(
            self.__class__.__name__, self.std, self.ndim_x, self.ndim_y
        )
