...
from NCP.cde_fork.density_simulation.BaseConditionalDensitySimulation import (
...
class SkewNormal(BaseConditionalDensitySimulation):
    ...
    def simulate_conditional(self, X):
        ...
        for i in range(X.shape[0]):
            rvs[i] = stats.skewnorm.rvs(
                skews[i], loc=locs[i], scale=scales[i], random_state=self.random_state
            )
